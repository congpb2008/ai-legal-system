...
function settingsPage(config,ocr){state.config=config;state.ocrConfig=ocr;return head('Server settings','Choose how this installation processes documents and questions.')+`<div class="panel"><h2>Processing mode</h2><p>Local mode searches and quotes documents on this machine. AI-assisted mode sends your question and relevant passages to the provider configured below.</p>${notice(state.status.mode==='ai'?'AI-assisted source selection is enabled.':'Local search is active. Scanned-page OCR is configured separately below.')}${state.status.mode==='ai'?button('Switch to local mode','local-mode'):'<a class="btn" href="#/upload">Continue with local mode</a>'}</div><form data-form="provider" class="panel"><h2>Optional AI provider</h2><p class="muted">Use an Ollama server with an embedding model and a chat model. Changing models may require rebuilding document search.</p>${input('base_url','Provider address','url',config.base_url,'required placeholder="http://localhost:11434/v1"')}${lanField('provider-lan',config.allow_lan)}<p class="muted">For another computer, use its LAN address, for example http://192.168.1.50:11434/v1. Ollama must listen on that computer’s network interface.</p>${input('model','Chat model','text',config.model,'required placeholder="qwen3:8b"')}${input('embedding_model','Embedding model','text',config.embedding_model||'bge-m3','required')}${input('api_key',config.has_api_key?'Provider key (leave empty to keep at the same address)':'Provider key (if required)','password','','autocomplete="off"')}<details><summary>Advanced options</summary><div class="form-grid">${input('timeout_seconds','Response timeout (seconds)','number',config.timeout_seconds||60,'min="1" max="120"')}${input('max_tokens','Maximum response tokens','number',config.max_tokens||4096,'min="256" max="8192"')}${select('reasoning_effort','Reasoning',[['none','None'],['low','Low'],['medium','Medium']],config.reasoning_effort||'none')}</div></details><label class="check"><input type="checkbox" id="provider-consent" required><span>I have approved this destination to process my organization’s questions and document passages.</span></label><div data-error></div><br><div class="actions">${button('Test connection','test-provider')}<button type="submit" class="btn primary">Save and enable</button></div><div id="provider-test"></div></form>${ocrSettings(ocr)}<div class="panel"><h2>Server operations</h2><p>Use the Legal Library Server application on the host machine to manage the service, port, HTTPS certificate, backups and recovery.</p>${button('Check server status','server-status')}<div id="server-status"></div></div>`;}
function lanField(id,checked){return `<label class="check"><input id="${id}" type="checkbox" ${checked?'checked':''}><span>Allow this provider on my local network</span></label>`;}
function ocrSettings(config){return `<form data-form="ocr" class="panel"><h2>Reading scanned PDFs (OCR)</h2>
 ...
 ${select('ocr-mode','OCR method',[['tesseract','Tesseract on this server'],['vision','Vision model (no Tesseract required)'],['tesseract_then_vision','Try Tesseract, then use vision if it fails']],config.mode)}
 <fieldset class="ocr-options" id="vision-settings" ${config.mode==='tesseract'?'hidden disabled':''}>
 ${input('ocr-address','Vision provider address','url',config.base_url,'placeholder="http://192.168.1.50:11434/v1"')}
 ${lanField('ocr-lan',config.allow_lan)}
 ${input('ocr-model','Vision model','text',config.model,'maxlength="200" placeholder="Name of your installed vision model"')}
 ${input('ocr-key',config.has_api_key?'Vision key (leave empty to keep at the same address)':'Vision API key (optional for Ollama)','password','','autocomplete="off"')}
 <div class="form-grid">${input('ocr-timeout','Time per scanned page (seconds)','number',config.timeout_seconds,'min="5" max="300"')}
 ${input('ocr-tokens','Maximum page output tokens','number',config.max_tokens,'min="512" max="32768"')}
 ${input('ocr-pages','Maximum vision pages per document','number',config.max_vision_pages,'min="1" max="200"')}</div>
 ${notice('Vision OCR can omit or invent text. Verify numbers, dates and tables against the original. Provider charges may apply to page images.')}
 ...
 </fieldset><div data-error></div><br><div class="actions">${button('Test OCR','test-ocr')}<button type="submit" class="btn primary">Save OCR settings</button></div>
...
function ocrBody(form){
 ...
 if(body.mode!=='tesseract'&&(!body.model||!body.base_url||!body.consent))throw new Error('Enter a vision provider address and model, and approve sending scanned page images.');
...
document.addEventListener('submit',event=>{
 ...
 withForm(form,async body=>{
  ...
  if(type==='ocr'){await api.post('/v1/ocr/config',ocrBody(form));state.status=await api.get('/v1/setup/status');toast('OCR settings saved. Retry documents that need attention.');await render();return}
  if(type==='provider'){body.allow_lan=form.querySelector('#provider-lan').checked;if(!body.api_key&&body.base_url.replace(/\/$/,'')===state.config.base_url.replace(/\/$/,''))delete body.api_key;await api.post('/v1/setup/config',body);await api.post('/v1/setup/complete');state.status=await api.get('/v1/setup/status');toast('AI provider enabled. New uploads will use this configuration.');await render()}
...
document.addEventListener('click',async event=>{
 ...
 try{
  ...
  if(action==='test-ocr'){const form=el.closest('form');if(!form.reportValidity())return;const body=ocrBody(form);el.disabled=true;$('ocr-test').innerHTML=notice('Checking OCR…');const result=await api.post('/v1/ocr/test',body);if($('ocr-test'))$('ocr-test').innerHTML=notice(result.message,result.reachable?'success':'warning');}
