...
ocr = root / 'ocr'
for required in ('tesseract.exe', 'tessdata/vie.traineddata', 'tessdata/eng.traineddata'):
    if not (ocr / required).is_file():
        raise SystemExit('Missing bundled OCR: run python packaging/prepare_ocr.py --seven-zip <7z.exe> first')
extra_binaries = []
...
a = Analysis([str(root / 'packaging/windows_entry.py')], pathex=[str(root/'backend')],
    ...
    datas=[(str(root/'frontend'), 'frontend'), (str(root/'design'), 'design'), (str(ocr), 'ocr')],
